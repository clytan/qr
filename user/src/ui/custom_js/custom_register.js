...
var eventHandler = {
        ...
        $.ajax({
          ...
          success: function (response) {
            if (response.success) {
              ...
              statusEl.html(
                `<span style="color: #10b981;">✓ ${discountText} applied! You save ₹${promoDiscount.toFixed(
                  2
                )}</span>`
              );
            } else {
              ...
              originalAmount = 0;
              statusEl.html(
                `<span style="color: #ef4444;">✗ ${response.message}</span>`
              );
              eventHandler.updatePayAmount();
            }
          },
          error: function () {
            ...
            originalAmount = 0;
            statusEl.html(
              '<span style="color: #ef4444;">✗ Error validating promo code</span>'
            );
            eventHandler.updatePayAmount();
          },
        });
  ...
  membershipTierEvents: () => {
      ...
      success: function (response) {
        ...
        if (response.status && response.data) {
          ...
          response.data.forEach(function (slab) {
            ...
            if (
              slabName.includes("normal") ||
              slabName.includes("bronze") ||
              slabName.includes("individual") ||
              slabName.includes("default")
            ) {
          ...
          if (window.slabMapping["normal"]) {
            ...
            console.log(
              "Default Normal tier slab ID set:",
              window.slabMapping["normal"]
            );
    ...
    $('input[name="user_tag"]').on("click", function () {
      ...
      if ($("#student_leader").val() === "yes") amount = 299;
      $("#pay-amount").text(amount);
      registerFunction.updateSubmitState();
    });
  },
  ...
  referenceEvents: () => {
    ...
    $("#reference_code").on("input", function () {
      ...
      if (!/^[A-Za-z0-9]{10}$/.test(code)) {
        $("#reference-status")
          .text("Reference code must be 10 characters (letters and numbers)")
          .css("color", "red");
  ...
  emailVerifyEvents: () => {
    $(document).on("click", "#verify-email-link", function (e) {
      ...
      if (
        typeof registerFunction !== "undefined" &&
        typeof registerFunction.sendOtp === "function"
      ) {
        ...
        $("#otp-status-msg")
          .css("color", "red")
          .text("Error: OTP function not available. Please refresh the page.");
      }
    });
    ...
    $("#submit-otp-btn").on("click", function () {
      ...
      var otp_id =
        $("#email-otp-modal").attr("data-otp-id") ||
  ...
  onChange: () => {
    ...
    eventHandler.updatePayAmount();
    $('input[name="user_tag"], input[name="user_type"], #student_leader').on(
      "change click",
      eventHandler.updatePayAmount
    );
  },
  ...
  submitEvent: () => {
    $("#register_form").on("submit", function (e) {
      ...
      const amount = registerFunction.calculateAmount(
        finalUserType,
        finalUserTag
      );
      formData.amount = amount; // Backend will verify this matches the tier
      console.log(
        "Form submission - User Type:",
        finalUserType,
        "Membership Tier:",
        finalUserTag,
        "Amount:",
          ...
          $.ajax({
            ...
            success: function (response) {
              ...
              if (response.status && response.session) {
                ...
                console.log(
                  "  Session:",
                  response.session.substring(0, 50) + "..."
                );
                console.log("  Order ID:", response.order_id);
                ...
                const redirectUrl =
                  "../backend/payment/intent.php?session=" +
                  session +
                  "&orderId=" +
                  orderId +
                  "&t=" +
...
var registerFunction = {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.status) {
          ...
          $("#reference-status")
            .text(response.message || "Invalid reference")
            .css("color", "red");
          registerFunction.referredByUserId = null;
          registerFunction.setReferenceValid(false);
        }
      ...
      error: function () {
        $("#reference-status")
          .text("Error validating reference")
          .css("color", "red");
        registerFunction.referredByUserId = null;
        registerFunction.setReferenceValid(false);
      },
  ...
  validateFullName: function () {
    ...
    if (!namePattern.test(name)) {
      $("#name-validation-status")
        .text(
          "Name must start with capital letter, followed by space and another capital letter (e.g., John D Smith)"
        )
        .css("color", "red");
      return false;
    }
  ...
  sendOtp: function (email, resend = false) {
    $("#otp-status-msg")
      .css("color", "black")
      .text(resend ? "Resending OTP..." : "Sending OTP...");
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.status) {
          ...
          if (otp_id) {
            ...
            $("#otp-status-msg")
              .css("color", "red")
              .text("Invalid OTP response. Please try again.");
          }
        } else {
          $("#otp-status-msg")
            .css("color", "red")
      ...
      error: function (xhr, status, error) {
        if (xhr.status === 0) {
          ...
          $("#otp-status-msg")
            .css("color", "red")
            .text("Server file not found. Please contact support.");
  ...
  getSelectedUserType: () => {
    return [
      $('input[name="user_type"]').filter(":checked").val() || null,
      $('input[name="user_type"]').filter(":checked").attr("data-tag") || "",
    ];
  },
  ...
  isFormComplete: () => {
    ...
    var collegeValid =
      !collegeRequired || (collegeRequired && collegeName !== "");
    ...
    var referenceValid =
      ...
      (/^[A-Za-z0-9]{10}$/.test(referenceCode) &&
        registerFunction.referenceValid);
    // Policy checkbox
